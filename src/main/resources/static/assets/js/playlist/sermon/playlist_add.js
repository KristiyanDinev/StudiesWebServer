...
async function searchSermons(query) {
    ...
    try {
        ...
        showError('Failed to search sermons. Please try again later.');
        dropdown.classList.add('d-none');
        dropdown.style.display = 'none';
    }
}
...
function displaySuggestions(sermons) {
    ...
    if (!sermons || sermons.length === 0) {
            ...
            item.innerHTML = `
                <div class="sermon-content">
                    ...
                    <div class="sermon-details">
                        <div class="sermon-name">${sermonName}</div>
        ...
        // Add scroll indicator if many sermons
        if (sermons.length > 5) {
            ...
            scrollIndicator.innerHTML = `
                <div class="text-center py-2">
                    <small class="text-muted">
                        ...
                        ${sermons.length} sermons - scroll for more
                    </small>
                </div>
            `;
    ...
    // Add custom CSS for sermon items
    if (!document.getElementById('dropdown-styles')) {
        ...
        style.textContent = `
            ...
            .sermon-item:hover .sermon-action {
                color: #667eea;
                opacity: 1;
                transform: scale(1.1);
            }

            .sermon-item:hover .sermon-icon {
            ...
            #sermon-dropdown::-webkit-scrollbar {
                width: 6px;
            }

            #sermon-dropdown::-webkit-scrollbar-track {
                background: #f1f3f4;
                border-radius: 3px;
                margin: 8px 0;
            }

            #sermon-dropdown::-webkit-scrollbar-thumb {
                background: linear-gradient(45deg, #667eea, #764ba2);
                border-radius: 3px;
            }

            #sermon-dropdown::-webkit-scrollbar-thumb:hover {
...
// Function to handle sermon selection
function selectSermon(sermon) {
    ...
    showSuccessMessage(`Selected: ${sermon.name}`);
...
searchInput.addEventListener('input', function() {
    ...
    // Reset selected sermon if user is typing
    selectedSermon = null;
...
uploadBtn.addEventListener('click', async function() {
    ...
    try {
        const res = await fetch('/playlists/sermon/add', {
            method: 'POST',
            body: formData
        })
        ...
        showError("Couldn't add this sermon to the playlist. Please try again later.");
    }
});
